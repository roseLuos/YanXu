...
- **本周科研时长**：汇总本周所有实验室打卡时段，同时展示本周最早到达、最晚离开与最长单次科研时长。
...
单次打卡连续超过 8 小时时，研序会提醒检查是否忘记打卡离开。可以按当前时间直接结束、继续计时，也可以填写实际离开的日期和时间，系统会自动重新计算时长。
...
- 展开后可在周视图上方的文本框快速新增任务，按回车保存；未指定日期的任务进入“待安排”；
- 点击顶部日期范围或任意一天的日期，可直接打开研序主窗口；
- 展开后点击右上角的太阳/月亮按钮，可切换白天与黑夜模式；研序会自动记住你的选择；
- 周视图中可以直接勾选任务，今天会使用蓝色强调；
- 点击灵动岛以外的任意位置，或点击展开面板右上角的向上箭头即可收起；
- 点击红色关闭按钮可彻底关闭灵动岛，需要时可从研序主页面顶部重新显示；
- 收起时可直接拖动整个灵动岛；展开时拖动左上角的图标和“本周计划”标题区域即可移动；支持拖到另一块显示器，并会自动保存目标显示器和位置；
- 右键点击可选择“回到屏幕顶部”，恢复默认位置；
- 灵动岛轮廓之外的透明区域不会拦截鼠标点击。

首次使用多块屏幕时，研序会优先将灵动岛放在有物理刘海的 MacBook 内置屏幕；拖到其他显示器后，以后会自动回到你保存的显示器和位置。
